...
export async function adminCreateStrategyConfigAction(
  formData: FormData
  ...
  try {
    ...
    if (!type || !name || !minAmount || !maxAmount || !minDays || !maxDays || !baseMinPercent || !baseMaxPercent) {
      return { success: false, error: "Missing required fields" };
    }

    const config: StrategyConfigData = {
      id: undefined, // No id for new config - will create new
      type: type as any,
      name,
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to create strategy config",
    };
  }
}
